...
public class OperatingSystem {
	...
	public synchronized int read(int pid, int virtAdr) {
		...
		//virtuelle Seitennummer ermitteln
		int virtualPageNum = getVirtualPageNum(virtAdr);
		// Prozess aus der Prozess ID ermitteln
		Process proc = getProcess(pid);
		// Offset ermitteln
		int offset = getOffset(virtAdr);
		// Bei dem jeweiligen Prozess in der Seitentabelle die Gesuchte ermitteln
		PageTableEntry pte = proc.pageTable.getPte(virtualPageNum);
		// zu der virtuellen Seitennummer die reele herausfinden
		int realAdress = pte.realPageFrameAdr + offset;
		// Prüfen ob diese im Hauptspeicher liegt wenn nicht Fehlerbehandlung
		if (!pte.valid) {
			pte = handlePageFault(pte, pid);
		}
		// Referenz flag auf false setzen
		pte.referenced = false;
		// in der Statistik hochzählen
		eventLog.incrementReadAccesses();

		// Mit der reelen Adresse aus Hauptspeicher lesen
	...
	private int getOffset(int virtAdr) {
		// Offset = Virtuelle Adresse modulo Seitengröße = „Positionsnummer“
		// innerhalb der Seite)
